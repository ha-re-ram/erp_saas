...
$stmt = $pdo->prepare("SELECT id, name FROM categories WHERE store_id = :store_id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($slug) || empty($price)) {
        ...
        $stmt = $pdo->prepare("SELECT id FROM products WHERE store_id = ? AND slug = ?");
        $stmt->execute([$store_id, $slug]);
        if ($stmt->fetch()) {
            $error = "A product with this URL slug already exists in your store.";
        } else {
            $stmt = $pdo->prepare("INSERT INTO products (store_id, category_id, name, slug, description, price, stock_quantity, low_stock_threshold, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if ($stmt->execute([$store_id, $category_id, $name, $slug, $description, $price, $stock_quantity, $low_stock_threshold, $image_url])) {
                // If success, redirect back to inventory list
                ...
                <form method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="name" id="name" class="form-control" required placeholder="e.g. Wireless Headphones">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-bold">Product Slug</label>
                            <input type="text" name="slug" id="slug" class="form-control" required placeholder="e.g. wireless-headphones">
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" class="form-control" rows="4" placeholder="Enter full specifications and describing traits..."></textarea>
                    </div>
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" name="low_stock_threshold" class="form-control" value="5" min="0" required>
                        </div>
                    </div>
                    ...
                    <button type="submit" class="btn btn-primary w-100 py-3 fw-bold fs-5">Add Product to Inventory</button>
                </form>
            </div>
        </div>
    </div>
